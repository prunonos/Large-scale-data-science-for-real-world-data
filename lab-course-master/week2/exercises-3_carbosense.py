...
df_co2minmax_melted =  df_co2minmax.melt(id_vars=['Zone_in_Zurich'],value_vars=['min','max'])
df_co2minmax_melted
...
# Insert code here
fig = px.histogram(df_co2minmax_melted,x='Zone_in_Zurich',y='value',
        barmode='group',height=400,color='variable')
...
# Insert code here
labels={'Timestamp':'Date','CO2':'CO2 (ppm)','Location_Name':'Sensor'}
title='CO2 level in October 2017'
dtick=24*60*60*1000
ticklabelmode='period'

fig = px.line(df_selection,x='Timestamp',y='CO2',color='Location_Name',
              labels=labels,
             title=title)
fig.update_xaxes(dtick=dtick,ticklabelmode=ticklabelmode)
...
def plot_co2(locations):
    ...
    fig = px.line(df_selection[df_selection.Location_Name == locations],
                  x='Timestamp',y='CO2',color='Location_Name',
                  labels=labels,
...
location_selector = widgets.RadioButtons(
    options=np.unique(df_selection.Location_Name),
