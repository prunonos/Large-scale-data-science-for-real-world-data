...
#
# We would normally use _magic_ commands in a PySpark kernerl with sparkmagic, e.g. `%spark config {...}` in order to start a session. Alternatively we exectute the cell magic commands programmatiacally using the IPython `run_cell_magic` API. This allows us to programmatically code your user (gaspar) name into the application name so that you can better track it. 
...
# %% language="spark"
# lang_df = df.filter(...).cache()

# %% [markdown]
# **TODO:** Construct the `ngram_model` by using `ngram_vectorize` with n_grams of length 1 to 3. Once we have this model, we can simply call its `transform` method to turn any corpus of documents into feature vectors. 

# %% language="spark"
# ngram_model = ...

# %% language="spark"
# ngram_model.transform(...)...
...
# > For LogisticRegression, you can set the following parameter values: `regParam=0.002`, `elasticNetParam=1`, `maxIter=10`
#
#
